...
### Average Daily Temperature Curves at JFK and LaGuardia Airports
...
airport |> 
  ...
  plot_ly(
    ...
    color = ~airport, text = ~label, hoverinfo = "text"
    ) |> 
  layout(
...
airport |> 
  ...
  plot_ly(
    x = ~snow, y = ~snwd, type = "scatter", mode = "markers",
    color = ~airport, text = ~label, hoverinfo = "text", alpha = 0.5
    ) |> 
  layout(
...
### Annual Number of Rainy or Snowy Days at JFK and LaGuardia Airports
...
airport |> 
  ...
  plot_ly(x = ~year, y = ~rain_snow_days_JFK, name = "JFK", type = "bar", text = ~label_JFK) |> 
